...
const char * const BITCOIN_CONF_FILENAME = "c25.conf";
const char * const BITCOIN_PID_FILENAME = "c25d.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "c25" is a composite category enabling all C25-related debug output
            if(ptrCategory->count(string("c25"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\C25Core
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\C25Core
    // Mac: ~/Library/Application Support/C25Core
    // Unix: ~/.c25core
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "C25Core";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/C25Core";
#else
    // Unix
    return pathRet / ".c25core";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty c25.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override c25.conf
