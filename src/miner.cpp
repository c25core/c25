...
// ***TODO*** ScanHash is not yet used in C25
//
...
{
    LogPrintf("C25Miner -- started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("c25-miner");
    ...
    try {
        ...
        while (true) {
            ...
            if (!pblocktemplate.get())
            {
                LogPrintf("C25Miner -- Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            CBlock *pblock = &pblocktemplate->block;
            IncrementExtraNonce(pblock, pindexPrev, nExtraNonce);

            LogPrintf("C25Miner -- Running miner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("C25Miner -- terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
        LogPrintf("C25Miner -- runtime error: %s\n", e.what());
        return;
    }
}
