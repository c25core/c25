...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a C25 address (e.g. %1)").arg("MN7sh43pV9cYbKHLArXxYcaTxPkwYz3Qmm"));
#endif
...
{
    // return if URI is not valid or is no c25: URI
    if(!uri.isValid() || uri.scheme() != QString("c25"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!BitcoinUnits::parse(BitcoinUnits::C25, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseBitcoinURI(QString uri, SendCoinsRecipient *out)
{
    // Convert c25:// to c25:
    //
    //    Cannot handle this later, because c25:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("c25://", Qt::CaseInsensitive))
    {
        uri.replace(0, 7, "c25:");
    }
...
{
    QString ret = QString("c25:%1").arg(info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::C25, info.amount, false, BitcoinUnits::separatorNever));
        paramCount++;
    }
...
void openConfigfile()
{
    ...
    /* Open c25.conf with the associated application */
...
boost::filesystem::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "C25 Core.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "C25 Core (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("C25 Core (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for "C25 Core*.lnk"
...
boost::filesystem::path static GetAutostartFilePath()
{
    std::string chain = ChainNameFromCommandLine();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "c25core.desktop";
    return GetAutostartDir() / strprintf("c25core-%s.lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = ChainNameFromCommandLine();
        // Write a c25core.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=C25 Core\n";
        else
            optionFile << strprintf("Name=C25 Core (%s)\n", chain);
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the C25 Core app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add C25 Core app to startup item list
